...
// Constantes específicas del kernel que no están en conexion.h
...
// Declaraciones de funciones de cliente sin incluir el header completo
...
// Tipos necesarios para comunicación (sin conflictos)
typedef struct
{
	int size;
	void* stream;
...
bool procesar_init_proc_syscall(char* nombre_proceso, int tamanio_proceso);
