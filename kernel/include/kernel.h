...
#define PCB_A_EJECUTAR 31
#define OPERACION_IO 202
#define READY 1
...
extern sem_t sem_kernel_memoria_hs;
extern sem_t sem_modulos_conectados;
extern sem_t sem_syscall_procesada;
extern sem_t hay_procesos_ready;

// Debe coincidir exactamente con las definiciones del I/O
typedef enum {
    IO_SOLICITUD = 1,
    IO_FINALIZACION = 2,
    IO_DESCONEXION = 3
...
// Estructura para manejar dispositivos I/O conectados
typedef struct {
    char* nombre;
    int socket_fd;
    bool conectado;
...
void* conectar_a_memoria_hilo(void* args);
...
bool enviar_handshake_io(int socket_io);
bool modulo_io_disponible();
bool procesar_syscall(char* syscall);
// Agrega este prototipo en kernel.h
...
// Funciones de comunicación con CPU
void* atender_cpu_dispatch();
void* atender_cpu_interrupt();
